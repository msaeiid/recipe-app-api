...
@extend_schema_view(
    ...
    list=extend_schema(
        # determine parameters user can send  to API
        parameters=[
            OpenApiParameter(
                'tags',
                OpenApiTypes.STR,
                description='Comma separated list of tags IDs for filter.',
            ),
            OpenApiParameter(
                'ingredients',
                OpenApiTypes.STR,
...
class RecipeViewSet(viewsets.ModelViewSet):
    ...
    def get_serializer_class(self):
        ...
        if self.action == 'list':
            # in list serializer doesn't show
            # description field an the rest shows
            return serializers.RecipeSerializer
        # we add custom action names upload_image
